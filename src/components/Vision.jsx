    ...
    <section style={{ padding: '140px 0', position: 'relative' }}>
      ...
      <div className="container">
        <div style={{ textAlign: 'center', marginBottom: '80px' }}>
          <motion.div className="sec-label" initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} style={{ justifyContent: 'center' }}>{t('vision.secLabel')}</motion.div>
          <motion.h2 initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: .1 }}
            ...
            {t('vision.h2a')} &amp; <span style={{ color: 'var(--blue2)' }}>{t('vision.h2b')}</span>
          </motion.h2>
        </div>
        ...
        <div className="vision-inner">
          {[
            {
              num: t('vision.num1'), title: t('vision.title1a'), accent: t('vision.title1b'), accentColor: 'var(--blue2)',
              text: t('vision.text1'),
              borderTop: 'linear-gradient(90deg,var(--blue),var(--blue2),transparent)',
              delay: .15, slideDir: 'left',
            },
            {
              num: t('vision.num2'), title: t('vision.title2a'), accent: t('vision.title2b'), accentColor: 'var(--emerald)',
              text: t('vision.text2'),
              borderTop: 'linear-gradient(90deg,var(--emerald),var(--emerald2),transparent)',
              delay: .25, slideDir: 'right',
            },
          ].map((block) => (
            <motion.div
              key={block.num}
              initial={{ opacity: 0, x: block.slideDir === 'left' ? -60 : 60 }}
